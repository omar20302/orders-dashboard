...
@st.cache_data(ttl=60, show_spinner=False)
def load_supabase_table(supabase_url, supabase_key, table_name="daily_order_items", company_id="", max_rows=50000, page_size=1000):
    """
    Load rows from Supabase REST API.
    ...
    table_name = str(table_name or "daily_order_items").strip()

    if not supabase_url or not supabase_key:
        raise ValueError("بيانات Supabase غير مكتملة. أضف SUPABASE_URL و SUPABASE_KEY في Streamlit Secrets.")
    if not table_name:
        raise ValueError("اسم جدول Supabase غير محدد.")
    ...
    while offset < max_rows:
        ...
        headers["Range"] = f"{offset}-{min(offset + page_size - 1, max_rows - 1)}"
        ...
        if str(company_id or "").strip():
            params["company_id"] = f"eq.{str(company_id).strip()}"

        response = requests.get(endpoint, headers=headers, params=params, timeout=30)

        if response.status_code not in [200, 206]:
            raise ValueError(f"Supabase error {response.status_code}: {response.text[:500]}")

        rows = response.json()
        if not isinstance(rows, list):
            raise ValueError("Supabase response غير متوقع. تأكد من اسم الجدول والصلاحيات.")
...
def format_supabase_time_for_sheet(value):
    if pd.isna(value):
        return ""
    text = str(value).strip()
    ...
    try:
        ...
        if pd.isna(ts):
            return text
        # If timezone-aware, convert to Riyadh then remove tz for display.
        try:
            if getattr(ts, "tzinfo", None) is not None:
                ts = ts.tz_convert("Asia/Riyadh").tz_localize(None)
        except Exception:
            pass
        hour = int(ts.hour)
        minute = int(ts.minute)
...
def format_supabase_date_for_sheet(value):
    if pd.isna(value):
        return ""
    text = str(value).strip()
    if not text or text.lower() in ["nan", "none", "null", "nat"]:
...
def normalize_supabase_orders_df(df):
    """
    Convert Supabase daily_order_items shape into the same Arabic/Google Sheet shape
    used by the current app, so all existing reports keep working.
    """
    ...
    # If pickup_time is empty but pickup_datetime exists, derive pickup_time/date from pickup_datetime.
    if "pickup_datetime" in src.columns:
        pickup_dt = pd.to_datetime(src["pickup_datetime"], errors="coerce")
    else:
        pickup_dt = pd.Series([pd.NaT] * len(src), index=src.index)

    if "courier_handoff_datetime" in src.columns:
        courier_dt = pd.to_datetime(src["courier_handoff_datetime"], errors="coerce")
    else:
        courier_dt = pd.Series([pd.NaT] * len(src), index=src.index)

    def col(name, default=""):
        if name in src.columns:
            return src[name].fillna(default).astype(str)
        return pd.Series([default] * len(src), index=src.index).astype(str)

    delivery_date = col("delivery_date")
    if "pickup_datetime" in src.columns:
        derived_dates = pickup_dt.apply(lambda x: x.strftime("%Y-%m-%d") if not pd.isna(x) else "")
        delivery_date = delivery_date.where(delivery_date.astype(str).str.strip().ne(""), derived_dates)

    pickup_time = col("pickup_time")
    if "pickup_datetime" in src.columns:
        derived_pickup_time = pickup_dt.apply(format_supabase_time_for_sheet)
        pickup_time = pickup_time.where(pickup_time.astype(str).str.strip().ne(""), derived_pickup_time)

    courier_time = col("courier_handoff_time")
    if "courier_handoff_datetime" in src.columns:
        derived_courier_time = courier_dt.apply(format_supabase_time_for_sheet)
        courier_time = courier_time.where(courier_time.astype(str).str.strip().ne(""), derived_courier_time)

    out = pd.DataFrame(index=src.index)

    out["رقم الطلب"] = col("order_number")
    out["معرف الطلب (Order Id)"] = col("order_id")
    out["اسم العميل"] = col("customer_name")
    out["اسم الشيف"] = col("branch_name")
    out["حالة الطلب"] = col("order_status")
    out["إجمالي الطلب بالكامل"] = col("order_total_amount")
    out["تاريخ التوصيل (Delivery Date)"] = delivery_date.apply(format_supabase_date_for_sheet)
    out["وقت الاستلام (Pickup Time)"] = pickup_time
    out["معرف العنصر (Item Id)"] = col("item_id")
    out["معرف الطبق (Dish Id)"] = col("dish_id")
    out["اسم الطبق / المنتج"] = col("product_name")
    out["نوع الحشوة (Variety)"] = col("variety_name")
    out["سعر الحشوة"] = col("variety_price", "0")
    out["ملاحظة للشيف"] = col("chef_note")
    out["سعر الحبة"] = col("unit_price", "0")
    out["الخصم"] = col("discount_amount", "0")
    out["الكمية"] = col("quantity", "1")
    out["إجمالي المنتج (Item Total)"] = col("product_total_amount", "0")
    out["وقت التسليم للمندوب"] = courier_time

    # Keep useful Supabase metadata for audit/debug/export if needed.
    optional_meta = {
        "Supabase ID": "id",
        ...
        "Source": "source",
        "Sheet Synced At": "sheet_synced_at",
        "Created At": "created_at",
    }
    for display_col, source_col in optional_meta.items():
        if source_col in src.columns:
            out[display_col] = src[source_col]
...
source_type = st.sidebar.radio("اختر المصدر", ["Supabase", "Google Sheet", "رفع ملف"], index=0, horizontal=True)

raw_df = None
load_error = None

if source_type == "Supabase":
    default_supabase_url = secret_value("SUPABASE_URL", "")
    default_supabase_key = secret_value("SUPABASE_KEY", "")
    default_table = secret_value("SUPABASE_TABLE", "daily_order_items")
    default_company_id = str(secret_value("SUPABASE_COMPANY_ID", "2") or "")

    st.sidebar.caption("يتم قراءة بيانات Supabase من Streamlit Secrets.")
    table_name = st.sidebar.text_input("اسم جدول Supabase", value=default_table)
    company_id_filter = st.sidebar.text_input("Company ID", value=default_company_id, help="اتركه فارغاً لو تريد كل الشركات.")
    max_rows = st.sidebar.number_input("أقصى عدد صفوف من Supabase", min_value=1000, max_value=100000, value=50000, step=1000)
    ...
    try:
        with st.spinner("جاري تحميل بيانات Supabase..."):
            supabase_raw = load_supabase_table(
            ...
            raw_df = normalize_supabase_orders_df(supabase_raw)
        with c_status:
            st.success("تم")
        if raw_df is not None and not raw_df.empty:
            st.sidebar.caption(f"Supabase rows: {len(raw_df):,}")
            missing_pickup = int(raw_df["وقت الاستلام (Pickup Time)"].astype(str).str.strip().eq("").sum()) if "وقت الاستلام (Pickup Time)" in raw_df.columns else 0
            if missing_pickup:
                st.sidebar.warning(f"تنبيه: {missing_pickup:,} صف بدون وقت استلام.")
    ...
    uploaded = st.sidebar.file_uploader("ارفع ملف TXT / CSV / Excel / JSON", type=["txt", "csv", "xlsx", "xls", "json"])
    if uploaded is not None:
        try:
            if uploaded.name.lower().endswith(".json"):
                uploaded_json = pd.read_json(uploaded)
                if isinstance(uploaded_json, pd.DataFrame) and "order_id" in uploaded_json.columns:
                    raw_df = normalize_supabase_orders_df(uploaded_json)
                else:
                    raw_df = uploaded_json.fillna("")
            else:
...
if load_error:
    st.error("لم أستطع تحميل البيانات. تأكد من إعدادات Supabase Secrets أو Google Sheet أو ارفع ملف مباشرة.")
    with st.expander("تفاصيل الخطأ"):
        st.code(load_error)
    st.stop()

if raw_df is None or raw_df.empty:
    st.markdown('<div class="note-box">اربط Supabase أو Google Sheet أو ارفع ملف الطلبات لبدء التحليل.</div>', unsafe_allow_html=True)
    st.stop()
...
# Supabase data quality hint before filters.
try:
    if source_type == "Supabase" and raw_df is not None and not raw_df.empty:
        pickup_missing_count = int(raw_df["وقت الاستلام (Pickup Time)"].astype(str).str.strip().eq("").sum()) if "وقت الاستلام (Pickup Time)" in raw_df.columns else 0
        if pickup_missing_count:
            st.warning(f"تنبيه بيانات Supabase: يوجد {pickup_missing_count:,} صف بدون وقت استلام. تقارير التأخير تحتاج pickup_time أو pickup_datetime.")
except Exception:
    pass
