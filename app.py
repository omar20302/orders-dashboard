...
if df is not None and len(df) > 0:
    ...
    with k1:
        render_kpi(tr("عدد الطلبات", "Orders"), format_int(active_unique_orders), "Unique Order Id", "#2563eb")
    with k2:
        render_kpi(tr("إجمالي الحشوات", "Total fillings"), format_int(total_fillings_qty), tr("حسب الكمية", "By quantity"), "#16a34a")
    with k3:
        render_kpi(tr("أعلى ساعة ضغط", "Peak hour"), top_hour.index[0] if len(top_hour) else "-", f"{format_int(top_hour.iloc[0]) if len(top_hour) else 0} {tr('طلب', 'orders')}", "#ea580c")
    with k4:
        render_kpi(tr("أعلى فرع", "Top branch"), top_branch.index[0] if len(top_branch) else "-", f"{format_int(top_branch.iloc[0]) if len(top_branch) else 0} {tr('طلب', 'orders')}", "#7c3aed")
    with k5:
